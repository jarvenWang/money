...
<div id="tabs-1" class="tab-itembox">
    ...
    <div class="in-total-box">
        <a  href="#" class="in-member-num">
            审核通过入款人数:<span>{{$pass_count}}</span>
        </a>
        <a  href="#" class="in-member-num">
            待审核人数:<span>{{$wait_count}}</span>
        </a>
        <a  href="#" class="in-member-num">
            没处理人数:<span>{{$start_count}}</span>
        </a>
        <a href="#"  class="in-member-num">
            记录总数:<span>{{$count}}</span>
        </a>
    </div>
    <!--  //总计 -->
    <!-- 列表数据  -->
    <table  id="myTable" class="tablesorter table  table-bordered ">
        ...
        @foreach ($datas as $data)
            <tr class="the{{$data->id}}">
                <td align="center">{{$data->id}}</td>
                <td align="center">{{$data->username}}</td>
                <td align="center">{{$data->account}}</td>
                <td align="center">{{$data->money}}</td>
                <td align="center">{{$data->type}}</td>
                <td align="center">{{$data->apply_time}}</td>
                <td align="center" class="ckstatus{{$data->id}}">{{$data->status}}</td>
                <td align="center" class="cktime{{$data->id}}">{{$data->check_time}}</td>
                <td align="center" class="ckremark{{$data->id}}">{{$data->remark}}</td>
                <td align="center"><a onclick="edit_log({{$data->id}})" class="com-smallbtn com-btn-color01 editer-btn" style="color: white;">审核</a><a onclick="del_log({{$data->id}})"  class=" com-smallbtn com-btn-color02 ml5 del-btn" style="color: white;">删除</a></td>
            </tr>
        @endforeach
        ...
        <div id="tabs-2" class="tab-itembox" style="display: none;">
            ...
            <table  id="myTable" class="tablesorter table  table-bordered ">
                ...
                @foreach ($results as $result)
                    <tr class="the{{$result->id}}">
                        <td align="center">{{$result->id}}</td>
                        <td align="center">{{$result->username}}</td>
                        <td align="center">{{$result->account}}</td>
                        <td align="center">{{$result->money}}</td>
                        <td align="center">{{$result->type}}</td>
                        <td align="center">{{$result->apply_time}}</td>
                        <td align="center" class="ckstatus{{$result->id}}">{{$result->status}}</td>
                        <td align="center" class="cktime{{$result->id}}">{{$result->check_time}}</td>
                        <td align="center" class="ckremark{{$result->id}}">{{$result->remark}}</td>
                        <td align="center"><a onclick="edit_log({{$result->id}})" class="com-smallbtn com-btn-color01 editer-btn" style="color: white;">审核</a><a onclick="del_log({{$result->id}})"  class=" com-smallbtn com-btn-color02 ml5 del-btn" style="color: white;">删除</a></td>
                    </tr>
                @endforeach
...
function fantwo(curr){
    ...
    function edit_log(e) {
        ...
        $("#can").on("click",function(){
            ...
            if(remark){
                $.post('/check_log',{'_token':'{{csrf_token()}}','status':'ok','id':e,'remark':remark},function(data) //第二个参数要传token的值 再传参数要用逗号隔开--}}
                {
        ...
        $("#cannot").on("click",function(){
            var remark =$("#remark").val();
            if(remark){
                $.post('/check_log',{'_token':'{{csrf_token()}}','status':'wait','id':e,'remark':remark},function(data) //第二个参数要传token的值 再传参数要用逗号隔开--}}
                {
    ...
    function del_log(val) {
        layer.confirm('确定删除吗？', {
            ...
            $.post('/del_log',{'_token':'{{csrf_token()}}','id':val},function(data) //第二个参数要传token的值 再传参数要用逗号隔开
            {
                if(data ==1){
                    layer.msg('删除成功');
                    $('tr').remove(".the"+val);
