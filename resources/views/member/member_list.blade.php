...
<div class="in-content-wrap">
    ...
    <div class="in-search-box clearfix">
        <form action="{{ route('_member-list') }}" method="get">
...
//            layer.alert($message);
    ...
    <table id="myTable" class="tablesorter table  table-bordered ">
        ...
        @forelse($data  as $d)
            <tr>
                <td align="center" id="userid{{ $d->id }}">{{ $d->id }}</td>
                <td align="center" id="name{{ $d->id }}">{{ $d->name }}</td>
                <td align="center" id="username{{ $d->id }}">{{ $d->username }}</td>
                <td align="center" id="email{{ $d->id }}">{{ $d->email }}</td>
                <td align="center" id="mobile{{ $d->id }}">{{ $d->mobile }}</td>
                <td align="center" id="remark{{ $d->id }}">{{ $d->remark }}</td>
                <td align="center" id="balance{{ $d->id }}">{{ $d->balance }}</td>
                <td align="center" id="off{{ $d->id }}">{{ $userStatus[$d->status] }}</td>
                <td align="center">
...
//        获取服务器传过来的第几页
        ...
        var counts ={{ $count or 0 }};
        var urls = "{{ route('_member-list') }}";

        if (page != getPages) {
            $.ajax({
                type: 'get',
                url: urls,
                data: {'p': curr,'where_json':'{{ $where }}' },
